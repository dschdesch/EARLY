...
CellInfo.datafile = upper(ds.ID.Experiment.ID.Name);
CellInfo.seqnr    = ds.ID.iDataset;
CellInfo.dsid     = [num2str(ds.ID.iCell) '-' num2str(ds.ID.iRecOfCell) '-' ds.StimType ] ;
...
try,
    if ischar(Param.ssronset),
        Param.ssronset = evalexpr(parseExpr(Param.ssronset, {'BurstDur', 'ISI'}), {ds.Stim.BurstDur, ds.Stim.ISI});
...
try,
    if ischar(Param.ssroffset),
        Param.ssroffset = evalexpr(parseExpr(Param.ssroffset, {'BurstDur', 'ISI'}), {ds.Stim.BurstDur, ds.Stim.ISI});
...
StimParam.repdur   = ds.Stim.ISI(1); %Repetition duration is always equal for both channels ...
StimParam.burstdur = ds.Stim.BurstDur([1, min(2, end)]); %Always two-element columnvector ...
StimParam.carfreq  = ds.Stim.Fcar(1:min(Nrec, end), :);
if all(ds.Stim.ModFreq == 0) | all(isnan(ds.Stim.ModFreq)), StimParam.modfreq = NaN;
else, StimParam.modfreq = ds.Stim.ModFreq(1:min(Nrec, end), :); end
if all(ds.Stim.BeatFreq == 0) | all(isnan(ds.Stim.BeatFreq)), StimParam.beatcarfreq = NaN;
else, StimParam.beatcarfreq = ds.Stim.BeatFreq(1:min(Nrec, end), :); end
% if all(ds.Stim.BeatModFreq == 0) | all(isnan(ds.Special.BeatModFreq)), StimParam.beatmodfreq = NaN;
% else, StimParam.beatmodfreq = ds.Special.BeatModFreq(1:min(Nrec, end), :); end
StimParam.nchan     = 2 - sign(ds.Stim.DAC);
StimParam.indepunit = ds.Stim.Presentation.X.ParUnit;
StimParam.indepval  = ds.Stim.Presentation.X.PlotVal(1:Nrec);
...
for n = 1:NSubSeqs,
    ...
    CalcData(n).fit.yn    = AdaptationFunc(C, PSTfit.BC)*WinDur*ds.Stim.Presentation.Nrep/1e3/NBin;
...
spt = spiketimes(ds);
SpkTimes = anwin(spt(SubSeq, :), AnWin);
