function [] = Monaural_Coincidence(Frange,Ctype)
...
ind = find(ANfreqs<=1.5);
allpairs = nchoosek(ind,2);%all possible fiber pairs
allpairsdfs = ANfreqs(allpairs);
octdiff = log2(allpairsdfs(:,1)./allpairsdfs(:,2));

ind = [];
indpairs = allpairs(abs(octdiff)<=Frange,:);
